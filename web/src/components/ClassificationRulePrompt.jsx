...
export function ClassificationRulePrompt({ transaction, onDismiss, onRemember }) {
  const [matchDescription, setMatchDescription] = useState(transaction?.description ?? '')
  const [matchAmount, setMatchAmount] = useState(
    transaction?.amount === null || transaction?.amount === undefined ? '' : String(transaction.amount),
  )
