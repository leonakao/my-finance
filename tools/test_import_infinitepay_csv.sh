...
CSV_PATH="${1:-inbox/infinitepay-2026-06.csv}"
SKIP_FUNCTIONS_SERVE="${SKIP_FUNCTIONS_SERVE:-0}"
FUNCTION_LOG="${TMPDIR:-/tmp}/finance-infinitepay-import-test-functions.log"
SIGNUP_RESPONSE="${TMPDIR:-/tmp}/finance-infinitepay-import-test-signup.json"
IMPORT_RESPONSE_1="${TMPDIR:-/tmp}/finance-infinitepay-import-test-import-1.json"
IMPORT_RESPONSE_2="${TMPDIR:-/tmp}/finance-infinitepay-import-test-import-2.json"
TRANSACTIONS_RESPONSE="${TMPDIR:-/tmp}/finance-infinitepay-import-test-transactions.json"
...
EMAIL="infinitepay-import-test-$(date +%s)-$$@example.com"
PASSWORD="Test123456!"
...
PAYLOAD=$(jq -nc --arg filename "$(basename "$CSV_PATH")" --arg csvText "$CSV_TEXT" '{filename: $filename, csvText: $csvText}')
BASE_COUNT=$(awk 'END { print NR - 1 }' "$CSV_PATH")

curl -sSf "$FUNCTIONS_URL/import-infinitepay-csv" \
  -H "apikey: $PUBLISHABLE_KEY" \
...
curl -sSf "$REST_URL/transactions?select=date,description,type,category,source,external_id&user_id=eq.$USER_ID&order=date.asc" \
  -H "apikey: $PUBLISHABLE_KEY" \
...
SALE_COUNT=$(jq '[.[] | select(.category == "Venda" and .type == "Receita" and .source == "InfinitePay")] | length' "$TRANSACTIONS_RESPONSE")
PIX_EXPENSE_COUNT=$(jq '[.[] | select(.description == "Pix Mayara Machado Guedes Vicente - Enviado" and .type == "Despesa")] | length' "$TRANSACTIONS_RESPONSE")

if [ "$IMPORTED_1" -ne "$BASE_COUNT" ]; then
  cat "$IMPORT_RESPONSE_1" >&2
  echo "Falha: a primeira importacao deveria identificar $BASE_COUNT linhas, mas identificou $IMPORTED_1." >&2
  exit 1
fi
...
if [ "$SALE_COUNT" -le 0 ] || [ "$PIX_EXPENSE_COUNT" -le 0 ]; then
  jq '.' "$TRANSACTIONS_RESPONSE" >&2
  echo "Falha: o importador nao classificou corretamente receitas de venda e Pix enviados." >&2
  exit 1
fi

echo "Teste OK."
echo "Usuario local: $EMAIL"
echo "Primeira importacao: $IMPORTED_1 identificadas, $INSERTED_1 inseridas"
